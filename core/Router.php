...
class Router{
    public static function route($url){
        ...
        $method_name = (isset($url[0]) && $url[0] !='') ? $url[0] : 'index';
        array_shift($url);
        ...
        if(method_exists($controller, $method)){
            ...
            die(' This method does not exists in the \"'.$controller.'\"');
        }
    }
    ...
    public static function redirect($location){
        if(!headers_sent()){
                header('Location:'.FROOT.$location);
                exit();
        }else{
                echo '<script type="text/javascript">';
                echo 'window.location.href="'.FROOT.$location.'";';
                echo '</script>';
                echo '<noscript>';
