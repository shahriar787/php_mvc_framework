                    ...
                    switch ($rule){
                        case 'min':
                            if(strlen($value) < $rule_value){
                                $this->addError(["{$display} is must be minimum of {$rule_value} characters.", $item]);
                            }
                            break;

                        case 'max':
                            if(strlen($value) > $rule_value){
                                $this->addError(["{$display} is must be maximum of {$rule_value} characters.", $item]);
                            }
                            break;
                        ...
                        case 'is_numeric':
                            if(!is_numeric($value)){
                                $this->addError(["{$display} has to be a number. Please use numeric value", $item]);
                            }
                            break;
    ...
    public function displayErrors(){
        ...
        foreach ($this->_errors as $error){
            if(is_array($error)){
                ...
                $html .= '<script>jQuery("document").ready(function(){jQuery("#'.$error[1].'").closest(".form-control").addClass("has-error");});</script>';
            }else{
